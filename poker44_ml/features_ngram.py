"""Hand n-gram features (fixed 331-token vocabulary, per-hand normalized).

Technique from the MIT-licensed poker44-handngram-miner lineage, as used by the
current leader family. Tokens: street initial + action code + pot-ratio bucket,
plus button-relative position tokens, bigrams and trigrams. Chunk feature =
token count summed over hands, divided by hand count (size-invariant).
"""
...
def ngram_features(chunk):
    n = float(len(chunk)) or 1.0
    totals: Counter = Counter()
    for hand in chunk:
        totals.update(_hand_ngram_doc(hand))
    out = {}
    for token in _NGRAM_VOCAB:
        out["schema_ngram_" + _sanitize_ngram_token(token)] = _safe_div(
            float(totals.get(token, 0.0)), n
        )
    return out
